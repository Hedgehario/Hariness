...
### 🦔 Phase 17: 画像アップロード機能 - ✅ 完了

**目標**: ハリネズミ個体のプロフィール画像をSupabase Storageにアップロードする機能を実装。

#### 17.1 Server Action実装

- [x] `uploadHedgehogImage` 関数を新規作成
- [x] ファイルサイズチェック（最大5MB）
- [x] ファイル形式チェック（JPEG, PNG, WebP）
- [x] 所有権確認（RLS相当）
- [x] Supabase Storageへのアップロード
- [x] `hedgehogs.image_url` の更新

#### 17.2 テスト作成

- [x] TC-HH-04: 画像アップロードバリデーションテスト
- [x] TC-HH-05: 不正ファイル形式/サイズテスト

---

### 🦔 Phase 18: 自動テスト導入 - ✅ 完了

**目標**: P0テストケースの自動化とテスト基盤の構築。

#### 18.1 テストフレームワーク導入

- [x] Vitest 導入（ユニット/統合テスト）
- [x] Playwright 導入（E2Eテスト）
- [x] カバレッジ計測（V8）
...
- **ユニット/統合テスト**: 42件パス（Vitest）
- **E2Eテスト**: 2件パス（Playwright）
