...
- [x] **5.4 お世話リマインダー**
    ...
    - [x] **リマインダー一覧・管理画面 (`/reminders`)**
    - [x] **リマインダー登録・編集モーダル**
    - [x] **ホーム画面への「追加」導線実装**
- [x] **5.5 共通レイアウト (ヘッダー/フッター)**
    - [x] `src/app/(main)/layout.tsx` 実装
    - [x] ログアウト、ナビゲーションメニューの共通化
    - [x] **要件定義に基づくUI修正**: ヘッダー構成変更、ボトムナビ項目変更
        - ヘッダー: 動的タイトル(左)、設定・通知アイコン(右)
        - ボトムナビ: ホーム、記録履歴、カレンダー、マップ
...
- [x] **7.2 イベント登録・編集 (C11)**
    ...
    - [x] UI: 登録・編集フォーム
- [x] **7.3 通院記録 (V10)**
    - [x] DB: `hospital_visits` テーブル確認/作成
    - [x] UI: 診療記録フォーム (薬・次回予約含む)
    - [x] **UI統一 & データ整備**
        - [x] 「今日の記録」用ハリネズミ選択UI
        - [x] ダミーデータ生成 (`/seed`)
        - [x] DBカラム名修正 (`record_date`)
