...
export async function seedData() {
  ...
  for (const name of DUMMY_HEDGEHOG_NAMES) {
      const { data, error } = await supabase.from('hedgehogs').insert({
          user_id: userId,
          name: name,
  ...
  if (!finalTargetId) {
       const { data: existing } = await supabase.from('hedgehogs').select('id').eq('user_id', userId).limit(1).single();
       if (existing) {
  ...
  const { data: wSample, error: wSampleError } = await supabase.from('weight_records').select('*').limit(1);
  if (wSampleError) {
  ...
  const { data: allVisits } = await supabase.from('hospital_visits').select('*').eq('hedgehog_id', finalTargetId);
  ...
  for (let i = 0; i < 5; i++) {
        ...
        const { data: existing } = await supabase.from('hospital_visits')
            .select('id')
